...
def cut_question_area(img, top_left, bottom_right):
    xmin, ymin = top_left
    xmax, ymax = bottom_right
    # 按照下边缘裁剪图像
    cropped_img = img[ymax:, :]  # 裁剪到 ymax 位置
    return cropped_img
...
if __name__ == "__main__":
    ...
    # 裁剪掉题干区域
    blurred_img = cut_question_area(main_image, top_left, bottom_right)


    # 保存结果，文件名从 main_image_path 中提取
    base_name = os.path.basename(main_image_path)  # 获取文件名（如 "19442.png"）
    output_path = os.path.join("output_blurOrCut", f"blurred_{base_name}")  # 保存路径
    os.makedirs("output_blurOrCut", exist_ok=True)  # 创建输出目录
